...
While it may sound pointless at first to transform our data table in a way that makes it have either more columns or rows (at least it did to me!), it actually is extremely useful for analyzing features of these data with relatively little code.

To illustrate, let's use the 'summarize' function from dplyr. Say we wanted to know the total number of M&Ms in the dataset. In wide format, we'd have to go across all the columns. In long format, these data are now in a single column:
```{r}
...
Note 1: the summarize function outputs a new tibble, which contains a new column we created (named 'mms_per_color') that contains the summed count data. You can continue to do calculations on this tibble (for example, if your data has many layers), but you cannot call your original tibble again within the same set of pipes.
...
Let's do some trickier stuff combining summarize, group_by, and mutate (to create a new column). Here we  compute the fraction of each color in each bag and store this in a new column:
```{r}
mms_long %>%
  group_by(BagID) %>%
